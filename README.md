  ...
  <body class="bg-gray-50 select-none">
    ...
    <script>
      ...
      function sendStateUpdate() {
        // Invia ogni aggiornamento di stato al bot (solo se siamo dentro Telegram WebApp)
        ...
        try {
          ...
          console.warn("Invio dati fallito", e);
        }
      }
      // ============ HELPERS PER INGRESSO DATI DA BOT/URL ============
      function isPlainObject(v) {
        return v && typeof v === "object" && !Array.isArray(v);
      }

      function applyIncomingData(payload) {
        // Merge sicuro: accettiamo solo campi previsti per evitare corruzione dello stato
        if (!isPlainObject(payload)) return;

        const allowed = {};

        if (isPlainObject(payload.weekData)) {
          // merge giorno per giorno
          allowed.weekData = { ...state.weekData };
          Object.keys(payload.weekData).forEach((d) => {
            if (isPlainObject(payload.weekData[d])) {
              allowed.weekData[d] = {
                ...allowed.weekData[d],
                ...payload.weekData[d],
              };
        ...
        if (Array.isArray(payload.savedWeights)) {
          allowed.savedWeights = payload.savedWeights.filter(
            (w) => w && typeof w.weight === "number"
          );
        }

        if (typeof payload.currentWeek === "number") {
          allowed.currentWeek = payload.currentWeek;
        }
        if (typeof payload.streak === "number") {
          allowed.streak = payload.streak;
        }
        if (typeof payload.totalWeeksCompleted === "number") {
          allowed.totalWeeksCompleted = payload.totalWeeksCompleted;
        }
        if (typeof payload.pointsForSgarro === "number") {
          allowed.pointsForSgarro = payload.pointsForSgarro;
        }
        if (Array.isArray(payload.weekHistory)) {
          allowed.weekHistory = payload.weekHistory;
        }
      ...
      async function initApp() {
        ...
        // Prova a caricare dallo storage cloud (priorità). Se non disponibile o vuoto,
        // fallback su IndexedDB locale.
          ...
          try {
            ...
            if (cloudRaw) {
              try {
                ...
                console.log("Stato caricato da Telegram CloudStorage");
              } catch (e) {
                console.warn("Parsing cloud state fallito", e);
              }
            }
          } catch (e) {
        ...
        if (!loadedFromCloud) {
          try {
            ...
            if (savedData) {
              ...
              console.log("Stato caricato da IndexedDB (fallback)");
            }
          } catch (e) {
        ...
        // ---- supporto: dati passati tramite query param "data" (base64 JSON) ----
        try {
          ...
          if (dataParam) {
            try {
              ...
              applyIncomingData(decoded);
            } catch (e) {
              console.warn("Invalid data param payload", e);
            }
          }
        } catch (e) {
        ...
        // Telegram WebApp init (se siamo dentro Telegram)
        if (tg) {
          try {
            tg.expand();
            tg.BackButton.show();
            ...
            // MainButton invia lo stato JSON al bot
            //tg.MainButton.setText("Invia report");
            //tg.MainButton.show();

            tg.onEvent("mainButtonClicked", () => {
              try {
                tg.sendData(JSON.stringify({ type: "state_report", state }));
              } catch (e) {
      ...
      function saveState() {
        // salva su IndexedDB (non bloccante)
        if (db) {
          db.save("winterGrindState", state).catch((e) =>
            console.warn("IndexedDB save failed", e)
          );
        }

        // salva su Telegram CloudStorage se disponibile (non bloccante)
        if (cloudAvailable()) {
          try {
            cloudSetItem("winterGrindState", JSON.stringify(state)).catch((e) =>
      ...
      function setState(updates) {
        ...
        saveState();
        render();
        // invia aggiornamento immediato per ogni cambio di stato
        sendStateUpdate();
      }
      ...
      function getStatusEmoji(points) {
        if (points >= 90) return "🔥 BESTIA";
        if (points >= 75) return "💪 SOLIDO";
        if (points >= 60) return "⚡ IN RIPRESA";
        return "🚨 RESET";
      }
      ...
      const icons = {
        trophy:
          '<svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M6 9H4.5a2.5 2.5 0 010-5H6m12 0h1.5a2.5 2.5 0 010 5H18M6 9v-4h12v4M6 9v7a6 6 0 0012 0V9M9 21h6"></path></svg>',
        calendar:
          '<svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="18" rx="2"></rect><path d="M16 2v4M8 2v4M3 10h18"></path></svg>',
        dumbbell:
          '<svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M6.5 6.5v11M17.5 6.5v11M2 8.5h2M2 15.5h2M20 8.5h2M20 15.5h2M7 7h10M7 17h10"></path></svg>',
        apple:
          '<svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M12 2.69l5.66 5.66a8 8 0 11-11.31 0z"></path><path d="M12 2v5"></path></svg>',
        trend:
          '<svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M23 6l-9.5 9.5-5-5L1 18"></path><path d="M17 6h6v6"></path></svg>',
        check:
          '<svg class="w-5 h-5 mx-auto" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 24 24"><path d="M20 6L9 17l-5-5"></path></svg>',
        circle:
          '<svg class="w-5 h-5 mx-auto" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"></circle></svg>',
        flame:
          '<svg class="w-12 h-12" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2c-1.1 0-2 .9-2 2 0 2.2-1.8 4-4 4-1.1 0-2 .9-2 2 0 4.4 3.6 8 8 8s8-3.6 8-8c0-1.1-.9-2-2-2-2.2 0-4-1.8-4-4 0-1.1-.9-2-2-2zm0 14c-2.2 0-4-1.8-4-4 0-.6.1-1.1.3-1.6.7.4 1.5.6 2.3.6 1.9 0 3.5-1.1 4.3-2.7.2.6.3 1.2.3 1.9 0 2.8-2.2 5-5 5z"></path></svg>',
        target:
          '<svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"></circle><circle cx="12" cy="12" r="6"></circle><circle cx="12" cy="12" r="2"></circle></svg>',
        award:
          '<svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="8" r="7"></circle><path d="M8.21 13.89L7 23l5-3 5 3-1.21-9.12"></path></svg>',
      };
      ...
      window.toggleActivity = function (day, activity) {
        ...
        // aggiorna stato (salvataggio e render gestiti da setState)
      ...
      window.resetWeek = function () {
        ...
        // SALVA LA SETTIMANA CORRENTE NELLO STORICO
        const weekToSave = {
          weekNumber: state.currentWeek,
          data: JSON.parse(JSON.stringify(state.weekData)), // copia profonda
          totalPoints: points,
      ...
      window.useSgarro = function () {
        if (state.pointsForSgarro > 0) {
          alert("Hai usato uno sgarro! Goditelo con moderazione.");
          setState({ pointsForSgarro: state.pointsForSgarro - 1 });
        } else {
          alert("Non hai punti sgarro disponibili.");
        }
      };
      ...
      window.saveWeight = function () {
        ...
        if (weight && weight > 0) {
          const newWeights = [
            ...state.savedWeights,
            { week: state.currentWeek, weight: weight },
          ];
      ...
      function renderDashboard() {
        const points = calculatePoints();
        const workouts = Object.values(state.weekData).filter(
          (d) => d.workout
        ).length;
        const diets = Object.values(state.weekData).filter(
          (d) => d.diet
        ).length;
        const cardio = Object.values(state.weekData).filter(
          (d) => d.cardio
        ).length;
        ...
        return `
                <div class="space-y-4">
                    <div class="bg-gradient-to-r from-orange-500 to-red-600 rounded-xl p-6 text-white shadow-lg">
                        <div class="flex items-center justify-between mb-4">
                            <div>
                                ...
                                <p class="text-orange-100">Gabriele - Settimana ${
                                  state.currentWeek
                                }</p>
                            </div>
                            ${icons.flame}
                        ...
                        <div class="bg-white/20 rounded-lg p-4 backdrop-blur">
                            <div class="flex justify-between items-center">
                                ...
                                <div class="text-right">
                                    <p class="text-2xl font-bold">${getStatusEmoji(
                                      points
                                    )}</p>
                                    <p class="text-sm text-orange-100">Streak: ${
                                      state.streak
                                    } settimane</p>
                                </div>
                            </div>
                        ...
                        <div class="w-full bg-gray-200 rounded-full h-3">
                            <div class="h-3 rounded-full transition-all duration-500 ${
                              points >= 90
                                ...
                                ? "bg-yellow-500"
                                : "bg-red-500"
                            }" style="width: ${points}%"></div>
                        </div>
                    </div>

                    <div class="grid grid-cols-3 gap-3">
                        <div class="bg-blue-50 rounded-xl p-4 text-center">
                            <div class="text-blue-600 flex justify-center mb-2">${
                              icons.dumbbell
                            }</div>
                        ...
                        <div class="bg-green-50 rounded-xl p-4 text-center">
                            <div class="text-green-600 flex justify-center mb-2">${
                              icons.apple
                            }</div>
                        ...
                        <div class="bg-purple-50 rounded-xl p-4 text-center">
                            <div class="text-purple-600 flex justify-center mb-2">${
                              icons.target
                            }</div>
                    ...
                    <div class="bg-gradient-to-r from-gray-800 to-gray-900 rounded-xl p-6 text-white shadow-lg">
                        <p class="text-sm text-gray-400 mb-2">💭 MANTRA DEL GRIND</p>
                    ...
                    <div class="bg-white rounded-xl p-4 shadow">
                        <div class="flex justify-between items-start mb-3">
                            <div>
                                ...
                                ${
                                  workout
                                    ? `<p class="text-sm text-gray-600">${workout.name} - ${workout.focus}</p>`
                                    : ""
                                }
                                ${
                                  isRestDay
                                    ? '<p class="text-sm text-gray-600">Riposo / Stretching</p>'
                                    : ""
                                }
                            </div>
          ...
          if (!isRestDay && (day === "wednesday" || day === "saturday")) {
            html += `
                        <button onclick="toggleActivity('${day}', 'cardio')" 
                                class="flex-1 py-3 rounded-lg font-semibold transition-all active:scale-95 ${
                                  data.cardio
      ...
      function renderProgress() {
        ...
        if (state.savedWeights.length > 0) {
          weightsHtml =
            '<div class="space-y-2"><p class="text-sm font-semibold text-gray-600 mb-2">Storico:</p>';
          ...
          if (state.savedWeights.length >= 2) {
            const diff = (
              state.savedWeights[state.savedWeights.length - 1].weight -
              state.savedWeights[0].weight
            ).toFixed(1);
            weightsHtml += `
                ...
                <div class="space-y-4">
                    ...
                    <div class="bg-white rounded-xl p-4 shadow">
                        ...
                        <div class="flex gap-2 mb-4">
                            <input type="number" step="0.1" placeholder="Es: 72.5" id="weightInput"
                        ...
                        <div class="space-y-2">
                            <div class="p-3 rounded-lg ${
                              points >= 90
                                ...
                                <p class="font-semibold">🔥 Bestia Mode</p>
                            ...
                            <div class="p-3 rounded-lg ${
                              state.streak >= 4
                                ...
                                <p class="font-semibold">⚡ 4 Settimane Streak</p>
                                <p class="text-xs text-gray-600">Mantieni la costanza (${
                                  state.streak
                                }/4)</p>
                            </div>
                            <div class="p-3 rounded-lg ${
                              state.currentWeek >= 8
                                ...
                                <p class="font-semibold">💎 Metà Inverno</p>
                                <p class="text-xs text-gray-600">Completa 8 settimane (${
                                  state.currentWeek
                                }/8)</p>
                            </div>
                            <div class="p-3 rounded-lg ${
                              state.totalWeeksCompleted >= 12
                                ? "bg-yellow-50 border-2 border-yellow-400"
                                : "bg-gray-100"
                            }">
                                <p class="font-semibold">👑 Winter Grind Completo</p>
                                <p class="text-xs text-gray-600">12 settimane totali (${
                                  state.totalWeeksCompleted
                                }/12)</p>
                            </div>
                        </div>
                    </div>
      ...
      function renderWorkouts() {
        ...
        let html =
          '<div class="space-y-3"><h2 class="text-2xl font-bold mb-4">Schede Allenamento</h2>';
        ...
        workouts.forEach((workout) => {
          ...
          workout.exercises.forEach((ex, idx) => {
            html += `
                        <div class="py-2 ${
                          idx < workout.exercises.length - 1 ? "border-b" : ""
                        }">
                            <div class="flex justify-between items-center">
                            ...
                            <p class="text-xs text-gray-500 mt-1">Recupero: ${
                              ex[2]
                            }</p>
                        </div>
                    `;
      ...
      function renderNutrition() {
        const meals = [
          ...
          },
          {
            ...
            items: [
              "40g noci/mandorle",
              "1 mela",
              '<span class="text-gray-500 italic">Alt: Yogurt greco + miele</span>',
            ],
            optional: true,
          },
          {
            ...
            kcal: "~900 kcal",
            items: [
              "100g riso/pasta (crudo)",
              "200g pollo/carne/pesce",
              "Verdure a volontà",
          ...
          },
          {
            ...
            kcal: "~ 400 kcal",
            items: [
              "200/300g Yogurt Greco",
              "20g Cornflakes",
              '<span class="text-gray-500 italic">Alt: 200g Tacchino + Cracker</span>',
            ],
          },
          {
            icon: "🍖",
            ...
            kcal: "~700 kcal",
            items: [
              "200/250g carne/pesce",
              "200g patate / 100g pane",
              "Verdure abbondanti",
              "1 cucchiaio olio EVO",
            ],
          },
        ...
        let html = `
                <div class="space-y-3">
                    ...
                    <div class="bg-gradient-to-r from-green-500 to-emerald-600 rounded-xl p-4 text-white shadow-lg mb-4">
                        <h3 class="font-bold text-lg mb-2">Target Giornaliero</h3>
                        <div class="grid grid-cols-3 gap-3 text-center">
                            <div><p class="text-2xl font-bold">180g</p><p class="text-xs text-green-100">Proteine</p></div>
                            <div><p class="text-2xl font-bold">250g</p><p class="text-xs text-green-100">Carboidrati</p></div>
                            <div><p class="text-2xl font-bold">60g</p><p class="text-xs text-green-100">Grassi</p></div>
                        </div>
                        <p class="text-center mt-3 text-sm">~2200 kcal/giorno</p>
                    </div>
            `;
        ...
        meals.forEach((meal) => {
          html += `
                    <div class="bg-white rounded-xl p-4 shadow">
                        <h3 class="font-bold mb-2">${meal.icon} ${
            meal.name
          }</h3>
                        <p class="text-sm text-gray-600 mb-2">${meal.kcal}${
            meal.optional ? " - Opzionale" : ""
          }</p>
        ...
        html += `
                <div class="bg-blue-50 rounded-xl p-4 mt-4">
                    <h3 class="font-bold mb-2 text-blue-800">💡 Tips Veloci</h3>
                    <div class="space-y-1 text-sm text-blue-900">
                        <p>✓ Meal prep domenica: cucina per 3-4 giorni</p>
                        <p>✓ Bevi 2.5-3L acqua al giorno</p>
                        <p>✓ 80% pulito, 20% flessibile weekend</p>
                        <p>✓ Pesati solo lunedì mattina a digiuno</p>
                        <p>✓ Obiettivo: +0.3-0.5kg al mese (massa pulita)</p>
                    </div>
                </div>
            </div>
            `;
      ...
      function renderCalendar() {
        ...
        // Funzione per determinare il colore in base al giorno
        function getDayColor(day, points) {
          // Domenica: max 3 punti (solo dieta)
          if (day === "sunday") {
            if (points >= 3) return "bg-green-400";
            return "bg-red-300";
          }
          // Mercoledì e Sabato: max 13 punti (cardio 10 + dieta 3)
          else if (day === "wednesday" || day === "saturday") {
            if (points >= 13) return "bg-green-400";
            if (points >= 10) return "bg-yellow-300";
            return "bg-red-300";
          }
          // Altri giorni: max 18 punti (palestra 15 + dieta 3)
          else {
        ...
        // RENDER STORICO SETTIMANE PRECEDENTI
        let historyHtml = "";
        if (state.weekHistory.length > 0) {
          historyHtml =
            '<h3 class="font-bold mb-3 text-gray-700">📅 Settimane Precedenti</h3>';

          // Inverti l'array per mostrare le più recenti prima
        ...
        return `
        <div class="space-y-4">
            <h2 class="text-2xl font-bold mb-4">📆 Calendario Completo</h2>
            
            <!-- SETTIMANA CORRENTE -->
            <div class="bg-white rounded-xl p-4 shadow-lg border-2 border-orange-500">
                <div class="flex justify-between items-center mb-3">
                    <h3 class="font-bold text-lg">🔥 Settimana ${
                      state.currentWeek
                    } (Corrente)</h3>
                    <span class="text-2xl">${getStatusEmoji(
                      calculatePoints()
                    )}</span>
                </div>
                ${currentWeekHtml}
            ...
            ${
              historyHtml
                ? `<div class="bg-white rounded-xl p-4 shadow">${historyHtml}</div>`
                : ""
            }
            ...
            <div class="bg-gradient-to-r from-purple-500 to-pink-600 rounded-xl p-6 text-white shadow-lg">
                <div class="text-center">
                    <p class="text-sm opacity-90 mb-2">🍕 Punti Sgarro Accumulati</p>
                   
                <h3 class="font-bold mb-3 text-blue-800">📊 Legenda Colori</h3>
        ...
        return `
                <div class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-2 py-2 max-w-md mx-auto">
                    <div class="flex justify-around">
                        <button onclick="setView('dashboard')" class="flex flex-col items-center p-2 rounded-lg transition-colors ${
                        ...
                        <button onclick="setView('tracker')" class="flex flex-col items-center p-2 rounded-lg transition-colors ${
                        ...
                        <button onclick="setView('workouts')" class="flex flex-col items-center p-2 rounded-lg transition-colors ${
                        ...
                        <button onclick="setView('nutrition')" class="flex flex-col items-center p-2 rounded-lg transition-colors ${
                          state.currentView === "nutrition"
                            ? "text-orange-600 bg-orange-50"
                            : "text-gray-600"
                        }">
                            ${icons.apple}
                        ...
                        <button onclick="setView('calendar')" class="flex flex-col items-center p-2 rounded-lg transition-colors ${
...
<button onclick="setView('progress')" class="flex flex-col items-center p-2 rounded-lg transition-colors ${
        ...
        document.getElementById("root").innerHTML = `
                <div class="max-w-md mx-auto bg-gray-50 min-h-screen pb-20">
                    <div class="p-4">
                        ${
                          tgUser
                            ? `<div class="mb-3 text-sm text-gray-500">Ciao, ${
                                tgUser.first_name || ""
                              }</div>`
                            : ""
                        }
